...
const EarningsSection = () => {
  ...
  return (
    <div className="bg-slate-50 py-16 sm:py-24 border-b border-slate-200">
      ...
      <div className="relative w-full">
        ...
        >
          {proofs.map((src, index) => (
            <div key={index} className="flex-shrink-0 transition-transform duration-300 hover:scale-[1.02]">
      ...
      {/* Styles for mobile-optimized button + subtle bounce animation (respects reduced-motion) */}
      <style>{`
        ...
        /* Apply bounce only on small screens to avoid distraction on desktop */
        ...
        @media (prefers-reduced-motion: reduce) {
          .subtle-bounce { animation: none !important; }
        }
      `}</style>

      {/* NEW BUTTON: centered below the slideshow (optimized for mobile) */}
      <div className="flex justify-center mt-6 px-4 sm:px-0">
        <a
          ...
          className={
            ...
            // - sm+: auto width and slightly larger text
            // plus bounce animation class applied only where defined above
            'subtle-bounce w-11/12 max-w-xl sm:w-auto inline-flex items-center justify-center px-6 py-3 sm:px-8 sm:py-3 bg-[#FF4500] hover:bg-[#ff5722] text-white font-bold rounded-xl sm:rounded-full shadow-lg transition-transform focus:outline-none focus:ring-4 focus:ring-orange-200 text-sm sm:text-base leading-tight text-center'
          }
        >
          <span className="block">
            YES I WANT TO BE
            <span className="hidden sm:inline"> </span>
            <span className="block sm:inline">FINANCIALLY INDEPENDENT</span>
          </span>
        </a>
      </div>
    </div>
